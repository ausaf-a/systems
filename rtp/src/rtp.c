...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    *count = length / MAX_PAYLOAD_LENGTH; 
    ...
    packet_t *packets = calloc(*count, sizeof(packet_t));
    if (packets == NULL) {
        fprintf(stderr, "Out of memory\n");
        exit(EXIT_FAILURE); 
    }
    for (int i = 0; i < *count; i++) {
        int start = MAX_PAYLOAD_LENGTH * i; 
        packets[i].type = DATA; 
        if (i == *count - 1 && length - i*MAX_PAYLOAD_LENGTH < MAX_PAYLOAD_LENGTH) {
        ...
        do {
           ...
           if (packet.type == DATA || packet.type == LAST_DATA) {
                if (packet.checksum == checksum(packet.payload, packet.payload_length)) {
                    if (buffer == NULL) {
                            buffer = malloc(packet.payload_length); 
                            if (buffer == NULL) {
                                // malloc failed 
                                fprintf(stderr, "Out of memory\n");
                                exit(EXIT_FAILURE); 
                            } 
                            memcpy(buffer, packet.payload, packet.payload_length); 
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        for (i = 0; i < array_length; i++) {
            ...
            if (!connection->ack) {
                i--; // this packet wasn't sent properly, try again
            }
            pthread_mutex_unlock(&connection->ack_mutex); 
