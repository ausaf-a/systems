...
void enqueue(queue_t *queue, pcb_t *process)
{
    ...
    else if (scheduler_algorithm == SRTF || scheduler_algorithm == SJF)
    {
        ...
        {
            while (curr->next != NULL && curr->next->time_remaining < process->time_remaining)
                curr = curr->next;

            // head -> 5 -> 10 -> 15 -> 20 -> 25 NULL
            // we have either reached the tail of the list, or we have found the insertion point
...
{
    pthread_mutex_lock(&current_mutex); 
    pcb_t* current_proc = current[cpu_id];
    pthread_mutex_unlock(&current_mutex); 
    
    current_proc->state = PROCESS_READY;
    enqueue(rq, current_proc); 
    schedule(cpu_id);  
}
...
extern void wake_up(pcb_t *process)
{
    ...
    if (scheduler_algorithm == SRTF)
    {
        ...
        for (unsigned int i = 0; i < cpu_count; i++)
        {
            ...
            if (current[i]->time_remaining > current[max]->time_remaining) {
                max = i; 
            }
        }
        pthread_mutex_unlock(&current_mutex);
        if (idle > 0 || process->time_remaining > current[max]->time_remaining) {
            // the process does not preempt any CPU
        } else {
            force_preempt(max); 
        }
    }
