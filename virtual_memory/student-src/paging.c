...
void proc_init(pcb_t *proc) {
    ...
    void* free_frame_addr = mem + free_pfn * PAGE_SIZE; 
    ...
    frame_table[free_pfn].protected = 1; 
    frame_table[free_pfn].process = proc; 
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    pte_t* pte = (pte_t *)(mem + (PTBR*PAGE_SIZE))+ vaddr_vpn(address); 
    ...
    */
   paddr_t ph_address = (paddr_t) (pte->pfn << OFFSET_LEN | vaddr_offset(address)); 
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        if (page_table[i].valid) {
            ...
            frame_table[page_table[i].pfn].referenced = 0; 
            frame_table[page_table[i].pfn].process = NULL; 
        }
